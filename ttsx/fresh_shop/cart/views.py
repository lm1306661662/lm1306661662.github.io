...
def cart(request):
    if request.method == 'GET':
        # 如果没有登录，则从session中取商品
        # 如果登录，还是从session中去取数据（保证数据库中的商品和session中商品一致）
        ...
        if session_goods:
            ...
            for goods in session_goods:
                # 获取商品对象
                # 前台需要商品信息，商品个数，商品的总价
                # 后台返回结构[[goods.objects,number,total_price],]
...
def place_order(request):
    if request.method == 'GET':
        ...
        for cart in carts:
            cart.total_prices = int(cart.nums) * int(cart.goods.shop_price)
        return render(request, 'place_order.html', {'carts': carts})


def delete(requset):
    if requset.method == 'POST':
        goods_id = requset.POST.get('id')
        goods = requset.session['goods']
        user_id = requset.session['user_id']
        for good in goods:
            if good[0] == int(goods_id):
                goods.remove(good)
                if user_id:
                    good = ShoppingCart.objects.filter(user_id=user_id, goods_id = goods_id).delete()
        requset.session['goods']=goods
